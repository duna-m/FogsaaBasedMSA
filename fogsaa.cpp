...
int m,n, expanded=0,score=0,lowerBound,a[3],b[3],ch[3],optp1,optp2,approximate=0,threshold, DNA, affine=0,Mt,Mst,Gp,Go,Ge,Ins,Del; // a=lower b=upper c=child index
float similarity=0.8;
// Mt == match score, Mst== Mismatch score, Gp== Gap penalty,, Go== Gap open penalty,, Ge== Gap extension penalty,

// structure definitiona
...
{	char ca,cb,pathend=0,loop=1;
    ...
    for(loop=1;loop<=1;loop++)
    {
        ...
        //Until here he created 2 files for to strands and created 2 arrays for 2 strands.
        //for protein
        if(DNA==2)
        {
        ...
        //matrix of m+1 * n+1 where each cell points to a cell struct defined previously.
        ...
        /// here is important to play with 30%
        th=ml*similarity;
        int worst=min(Ins,Del);
        if(DNA==1)
        {
            if(affine==0)
                threshold=th*Mt+(sl-th)*Mst+worst*(ml-sl);
            else
        ...
        if(m!=0 && n!=0)
        {
            do
            {
                ...
                while((curp1<=m-1) || (curp2 <=n-1))
                {
...

                    {
                        ...
                        if(curp1<=m-1 && curp2<=n-1)
                        {
                            ...
                            if(affine==0)
                            {
                                ...
                                calculate_score(present+Ins,&a[1],&b[1],curp1,curp2+1);
                                calculate_score(present+Del,&a[2],&b[2],curp1+1,curp2);
                            }
                    ...
                    else if((type_total==3)||(type_total==5)||(type_total==6))
                    {
                        ...
                        if(new_type==1)
                        {
                            ...
                            {
                                if(affine==0)
                                    calculate_score(present+Ins,&next_lower,&next_upper,curp1,curp2+1);
                                else
                                {
                            ...
                            {
                                if(affine==0)
                                    calculate_score(present+Del,&next_lower,&next_upper,curp1+1,curp2);
                                else
                ...
                if(((cpl>(1-similarity)*ml)&&(lowerBound<threshold))||(new_upper<threshold))
                {
                    printf("The given sequences are not globally similar(below 30%% of similarity)-- try local allignment --\n");
                    printf(" The score below is near optimal\n");
                    printf("The threshold== %d\n",threshold);
...
//calculate min max depending on all the rest matches or mismatches
void calculate_score(int p,int* l,int * u,int p1,int p2)
{
    if(DNA==2)
    {
    ...
    if((m-p1)<=(n-p2))
    {
        *l=p+((m-p1)*Mst+Ins*((n-p2)-(m-p1)));
        *u=p+((m-p1)*Mt+Ins*((n-p2)-(m-p1)));
    }
    else
    {
        *l=p+Mst*(n-p2)+Del*((m-p1)-(n-p2));
        *u=p+(n-p2)*Mt+Del*((m-p1)-(n-p2));
    }
}
